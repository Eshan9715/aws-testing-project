...
const BottomNav = ({children}) => {
  ...
  return (
      <>
        <main>{children}</main>
        <div class="mdd:hidden fixed bottom-0 left-0 z-50 w-full h-16 bg-white dark:bg-gray-400">
          <div class="grid h-full max-w-lg grid-cols-4 mx-auto font-medium">
            ...
            <div className={` w-full flex flex-col justify-center shadow:md items-center p-2 gap-1 m-1 text-center `}  key={tag.id}>
                {tag.icon}
                <p className='flex xl:text-base text-[14px] text-start justify-start'>{tag.val}</p> 
            </div>
            </NavLink>
          ))}
